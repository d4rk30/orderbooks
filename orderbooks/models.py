...
class Good(db.Model):
    ...
    types = db.relationship('Type',secondary = association_table,back_populates = 'goods') # 每个商品有多个所属分类

# 商品归属的分类
class Type(db.Model):
    id = db.Column(db.Integer,primary_key=True,autoincrement=True,unique=True)
    name = db.Column(db.String(20),nullable=False,unique=True)# 商品分类名称
    type_image = db.Column(db.Text,nullable=False)# 分类的展示的图标

    goods = db.relationship('Good',secondary = association_table,back_populates = 'types') # 每个分类下有多个产品
...
class Order(db.Model):
    ...
    status = db.Column(db.Integer,nullable=False,default=1)# 订单状态，1:等待确认，2：备货中，3，已发货,0:已取消订单
...
# 首页模块开关
class HomeSwitch(db.Model):
    id = db.Column(db.Integer,primary_key=True,autoincrement=True,unique=True)
    banner_status = db.Column(db.Boolean,nullable=False,default=False)# Banner开启状态
    type_status = db.Column(db.Boolean,nullable=False,default=False)# Banner开启状态
